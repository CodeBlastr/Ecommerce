  ...
  <div id="orderTransactionItems" class="orderTransactionItems">
	...
	foreach ($orderItems as $i => $orderItem) :
		...
		if(!empty($orderItem['OrderItem']['arb_settings'])) :
			$arbUnserialized = @unserialize($orderItem['OrderItem']['arb_settings']);
			if ($arbUnserialized !== false) :
		...
		if(isset($unserializeValue)) :
       		foreach($unserializeValue as $key => $value) :
       	 		if(is_array($value) && $key == 'Chained') :
       	 			foreach($value as $setting => $val) :
        	 			echo $this->Form->hidden("OrderPayment.{$setting}" , array('value' => $val));
        	 		endforeach;
        	 	else : ?>
    ...
    <div class="orderTransactionItem">
      <?php
		echo $this->element('thumb', array('plugin' => 'galleries', 'model' => 'CatalogItem', 'foreignKey' => $orderItem['OrderItem']['catalog_item_id'], 'thumbSize' => 'small', 'thumbLink' => '/catalogs/catalog_items/view/'.$orderItem['OrderItem']['catalog_item_id'])); 
		echo '<span class="orderTransactionQuantity">' . $orderItem['OrderItem']['quantity'] . ' qty, of </span> ' . $orderItem['OrderItem']['name']?>
    </div>
    <?php
    	endif; //isset($unserializeValue) ?>
    <?php
	if($enableShipping) :
